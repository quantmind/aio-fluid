...
A TaskBroker is responsible for queuing task runs and storing task information. A subclass has to implement every abstract method below, grouped here by the concern each one serves.
...
The return type of `lock` is currently the lock class of the `redis` client, so a broker built on a different backend has to return an object satisfying that type. Only the async context manager protocol is used at runtime.
