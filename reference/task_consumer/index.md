...
Workers added via `add_workers` are started by `startup`, therefore they only run in a process which consumes the task queue.

The async dispatcher is added as an async context manager instead, so it is started by `__aenter__` and runs in every process which uses the task manager. A cpu bound task runs in a process of its own, which executes a single task and never starts the consumer workers, and it still needs to dispatch the lifecycle events its plugins and handlers subscribe to.
