...
Pass a list as the value to match any of its values with `eq`, or none of them
with `ne`:
...
The list is sent as a single array parameter, `= ANY` for `eq` and `<> ALL`
for `ne`, rather than one parameter per value, so it can hold any number of
values: a query can carry at most 32767 parameters. An empty list matches no
row with `eq` and excludes none with `ne`.
